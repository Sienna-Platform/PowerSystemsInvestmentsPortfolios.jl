...
"""Get [`MaximumCapacityRequirements`](@ref) `max_capacity_mw` as a bare number in the requested `units` (e.g. domain-provided units such as `MW`). Returns a bare number only when such a method is registered; otherwise returns the quantity wrapper. For the unit-bearing value see [`get_max_capacity_mw_unitful`](@ref)."""
...
"""Get [`MaximumCapacityRequirements`](@ref) `max_capacity_mw` as a unit-bearing quantity in the requested `units` (e.g. `MW`). For a bare number see [`get_max_capacity_mw`](@ref)."""
