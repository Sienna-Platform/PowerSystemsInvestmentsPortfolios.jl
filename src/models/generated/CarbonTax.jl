...
"""Get [`CarbonTax`](@ref) `tax_dollars_per_ton` as a bare number in the requested `units` (e.g. domain-provided units such as `MW`). Returns a bare number only when such a method is registered; otherwise returns the quantity wrapper. For the unit-bearing value see [`get_tax_dollars_per_ton_unitful`](@ref)."""
...
"""Get [`CarbonTax`](@ref) `tax_dollars_per_ton` as a unit-bearing quantity in the requested `units` (e.g. `MW`). For a bare number see [`get_tax_dollars_per_ton`](@ref)."""
